#!/usr/bin/env python3
"""Render governed-effect tongue proposal records into warm Discord-readable text."""
...
def render_claim(record: dict[str, Any]) -> str:
    ...
    claim = record.get("payload", {}).get("claim") or record.get("payload", {}).get("utterance") or "a claim"
...
def render_doubt(record: dict[str, Any]) -> str:
    ...
    doubt = record.get("payload", {}).get("doubt") or record.get("payload", {}).get("utterance") or "this may need checking"
...
def render_intend(record: dict[str, Any]) -> str:
    ...
    return (
        ...
        "This is only a proposal until the governed runtime admits, verifies, approves, and commits it."
    )
...
def render_remember(record: dict[str, Any]) -> str:
    ...
    candidate = payload.get("memory_candidate") or payload.get("utterance") or "a memory candidate"
...
def render_boundary(record: dict[str, Any]) -> str:
    ...
    payload = record.get("payload", {})
    boundary = payload.get("boundary") or payload.get("utterance") or "I cannot safely continue as-is"
...
def main() -> None:
    ...
    parser.add_argument("path", nargs="?", help="JSON or JSONL proposal records; stdin if omitted")
    parser.add_argument("--limit", type=int, default=0, help="limit number of records rendered")
