#!/usr/bin/env python3
"""Tiny parser for the v0 AI-native tongue public speech subset.

Parses only: need, claim, doubt, remember, boundary.
It intentionally does not parse file-write `intend` records; those should be
JSON-Schema validated first in v0 because they can lead to real effects.
"""
...
NEED_RE = re.compile(
    r'^need\s+(?P<capability>\S+)\s+target:"(?P<target>[^"]+)"(?:\s+because:"(?P<because>[^"]+)")?$'
)
CLAIM_RE = re.compile(r'^claim\s+"(?P<claim>[^"]+)"(?:\s+evidence:\[(?P<evidence>[^\]]*)\])?$')
DOUBT_RE = re.compile(r'^doubt\s+(?P<doubt>.+?)(?:\s+because:"?(?P<because>[^"\n]+)"?)?$')
REMEMBER_RE = re.compile(r'^remember\s+(?P<lifespan>session|project|durable)\s+"(?P<candidate>[^"]+)"$')
