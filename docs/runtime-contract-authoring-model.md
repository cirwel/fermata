...
### Human policy → canonical Scope record

A human writes (parsed by ``src/fermata/policy_parser.py``):
...
```json
{
  ...
  "approvals": [{"authority": "human", "condition": "effect.kind == \"file.write\""}],
  "audit":     {"retain": ["trace", "input_hash", "output_hash", "actor", "approval"]}
}
```

This is the human surface. It owns: `scope_id`, `resources`, `capabilities`,
...
**Human inline grant.** The human-policy parser has no `approval grant
...` construct — the human surface declares *requirements*
(`approval require human if ...`), not granted approvals. A grant only
exists at runtime, when an operator passes `approval_granted=True` to the
adapter evaluator. Tested in `run_self_tests` as
`human_policy_cannot_inline_grant`.
